...
public class Redis {
     ...
     *
     * 2：说一下Redis复制的过程（代办）
     * 旧版同步：当一个客户端执行了slaveOf ip和端口时，就进行主从复制的操作。
     * 进行主从复制的时候，需要从节点向主节点发送一个sync命令，主节点接收到这个命令之后就会fork一个子进程去生成一个RDB文件。RDB文件生成之后就会
     * 发生给从节点，主节点还会记录执行fork命令之后接收到的写请求，也发给从节点，从节点接收到RDB文件和命令，更新到和主节点一样的数据，这样就可以达到主从一致性了。
     * 当然，达到了主从一致性之后，当主节点再处理写命令时，主从节点又会变得不一致了，这时候，主节点会主动发送写命令给从节点，冲节点执行之后又可以重新恢复一致。
     *
     * 旧版的缺点：网络中断了，会怎样。
     *
     * 新版全部
